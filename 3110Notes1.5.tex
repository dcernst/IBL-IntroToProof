...
By using our abbreviations for the logical connectives and quantifiers, we symbolically represent mathematical propositions.
...
This proposition can be denotes symbolically as $(\exists x)(x^2-1=0)$.
...
They can be represented symbolically as $(\forall x)(x\in\mathbb{N}\implies (\exists y)(y\in\mathbb{N}\implies y<x))$ or more simply as $(\forall x\in\mathbb{N})(\exists y\in\mathbb{N})(y<x)$.
...
\begin{exercise} Convert the following statements into statements using only logical symbols.  Unless stated otherwise, assume that the universe of discourse is the set of real numbers.
\begin{enumerate}
...
%It is also useful to be able to covert from a symbolic statement into an ordinary English sentence.

\begin{exercise} Unpack the following symbolic propositions into words: 
\begin{enumerate}
\item $(\forall n \in \mathbb{N})(n^2 +1 \neq 0)$
\item $(\exists N \in \mathbb{N})(\forall  n > N)(\frac{1}{n} < 0.01)$
\end{enumerate} 
\end{exercise}

%\begin{exercise}  Unpack the following statement into words: $$\forall \varepsilon >0, \exists \delta >0,  (\forall x, |x-c|< \delta \implies |f(x)-f(c)|< \varepsilon).$$ \end{exercise}
...
\begin{exercise}
%For each of the following statements, (i) unpack the statement into words, (ii) find the negation of the statement, (iii) state whether the original statement or its negation is true, (iv) prove the statement you asserted to be true.
