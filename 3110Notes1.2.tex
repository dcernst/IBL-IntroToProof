...
%\item Using a truth table find an expression equivalent to $\neg (A \wedge B)$ involving ``or'' ($\vee$).  (Recall that two logical statements are equivalent if they agree in truth value for all cases.)

%\item Using a truth table find an expression equivalent to $\neg (A \wedge B \wedge C)$ involving ``or'' ($\vee$).

%\item Describe the practical meaning of $\neg (A \vee B)$ using sentences.

%\item Using a truth table find an expression equivalent to $\neg (A \vee B)$ involving ``and'' ($\wedge$)
