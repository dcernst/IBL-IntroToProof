...
\begin{definition}
If $A$ and $B$ are sets, then we say that $A$ is a \textbf{subset} of $B$, written $A\subseteq B$, if every element of $A$ is also an element of $B$.
\end{definition}
...
\begin{exercise}
Provide two examples of proper subsets of $[0,1]$.
\end{exercise}

Here are some more definitions.  In each case, take $U$ to be the universe of discourse.

\begin{definition}
The \textbf{union} of the sets $A$ and $B$ is
$A \cup B =\{x\in U : x\in A \mbox{ or } x\in B \}$
\end{definition}

\begin{definition}
The \textbf{intersection} of the sets $A$ and $B$ is
$A \cap B =\{x\in U : x\in A \mbox{ and } x\in B \}$
\end{definition}

\begin{definition}
The \textbf{set difference} of the sets $A$ and $B$ is
$A \setminus B =\{x\in U : x\in A \mbox{ and } x\notin B \}$
\end{definition}
...
%\begin{problem} Let $X, A, B$ be sets.  If $X\setminus B \subset X \setminus A$, what additional hypothesis is needed to conclude $A \subset B$? Prove your answer. \end{problem}
%
%\begin{definition}
%For each natural number $n$, let $S_n$ be a subset of some set $X$. Define $$\bigcup_{n=1}^{\infty}S_n= \{ x \in X : \mbox{ There is an }n \mbox{ such that } x \in S_n \}.$$
%\end{definition}
%
%\begin{definition}
%For each natural number $n$, let $S_n$ be a subset of some set $X$. Define $$\bigcap_{n=1}^{\infty}S_n= \{ x \in X : \mbox{ For all }n \mbox{, }x \in S_n \}.$$
%\end{definition}
%
%\begin{problem}
%Find what the following set is equal to, if $S_n = \{x \in \mathbb{R} : n-1<x<n \}$
%$$\bigcup_{n=1}^{\infty}S_n = $$
%\end{problem}
%
%\begin{problem}
%Find what the following set is equal to, if $S_n = \{x \in \mathbb{R} : -\frac{1}{n}<x< \frac{1}{n} \}$
