...
Before tackling a proof of Theorem~\ref{thm:sqrt2}, we need a few tools.  In particular, we'll need the following result known as the Division Algorithm, but we won't worry about proving it.  Instead, we will take it for granted and use it the proof of Theorem~\ref{thm:Euclid}.

\begin{theorem}[Division Algorithm]
Suppose that $m,n\in\mathbb{N}$.  Then there exists unique $q,r\in\mathbb{N}$ such that $m=nq+r$ with $0\leq r<n$.
\end{theorem}

See if you can prove the following theorem, which is known as Euclid's Lemma.%First, recall Problem~1.15, which asked us to prove or provide a counter example to the following statement:
%\begin{quote}
%Assume $a$ and $n$ are integers.  If $a$ divides $n^2$, then $a$ divides $n$.
%\end{quote}
%It turns out that this statement is false.  However, what if $a$ is prime?  Before tackling this question, let's take a further step back.  Try proving the next theorem.
