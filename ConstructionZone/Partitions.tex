...
\begin{problem}
For each relation in Problem~\ref{prob:lots of relations}, determine whether the corresponding collection of sets of relatives forms a partition of the given set.
\end{problem}
...
\begin{definition}
Let $A$ be a set and $\Omega$ any collection of subsets of $A$ (not necessarily a partition).  If $a,b\in A$, we will define $a$ to be $\Omega$-related to $b$ if there exists $R\in \Omega$ that contains both $a$ and $b$.  This relation is denoted by $\sim_{\Omega}$ and is called the \textbf{relation on $A$ associated to $\Omega$}.
\end{definition}

This definition may look more awkward than the underlying concept.  The idea is that if two elements are in the same subset, then they are related.  For example, when my kids pick up all their toys and put each toy in the appropriate toy bin, we say that two toys are related if they are in the same bin.

Notice that we have two similar-looking notations: $\Omega_{\sim}$ and $\sim_{\Omega}$.  As a reminder:
\begin{enumerate}[label=\textrm{(\alph*)}]
...
\item The relation induced by the collection of subsets $\Omega$ is denoted by $\sim_{\Omega}$.
\end{enumerate}
...
\begin{problem}
Let $A$ and $\Omega_{\sim}$ be as in Problem~\ref{prob:find sim from Omega}. Find the relation on $A$ associated to $\Omega_{\sim}$ and compare with what you obtained for $\sim$ in Problem~\ref{prob:find sim from Omega}.
\end{problem}
