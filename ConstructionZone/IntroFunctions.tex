...
Let $X$ and $Y$ be two nonempty sets.  A \textbf{function $f$ from $X$ to set $Y$} is a relation from $X$ to $Y$ such that for every $x\in X$, there exists a unique $y\in Y$ such that $(x,y)\in f$. The set $X$ is called the \textbf{domain} of $f$ and is denoted by $\dom(f)$.  The set $Y$ is called the \textbf{codomain} of $f$ and is denoted by $\codom(f)$ while the subset of the codomain defined via
\[
...
It is important to point out that not every function, even one involving sets of numbers, can be described using a formula! Despite your prior experience, functions that can be represented succinctly using a formula are rare.
...
\begin{problem}
Determine whether each of the following functions is one-to-one, onto, both, or neither.  In each case, you should provide a proof or a counterexamples as appropriate.
\begin{enumerate}[label=\textrm{(\alph*)}]
...
\item Suppose that $S$ is a function. Recall that a function is a set of ordered pairs, so this makes sense.  Is $\pi_{1}$ injective? Is $\pi_{1}$ surjective?  How about $\pi_{2}$?
\end{enumerate} 
\end{problem}
...
Various ways of not being well defined:
\begin{enumerate}[label=\textrm{(\alph*)}]
\item $f:\{1,2,3\}\to \{1,2,3\}$ defined via $f(x)=x-1$.
\item $f:\mathbb{Q}\to \mathbb{Z}$ defined via $f(a/b)=a+b$.
\item Something like $f([x])=???$
\end{enumerate}
