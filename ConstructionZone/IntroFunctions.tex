...
\begin{figure}
\centering
\begin{tikzpicture}[>=stealth,  bullet/.style={fill=black, circle, minimum width=1pt, inner sep=1pt}, projection/.style={->, thick, shorten <=2pt, shorten >=2pt}, every fit/.style={ellipse, draw, inner sep=0pt}]
...
\begin{theorem}
Let $A$ be a nonempty set and suppose $\sim$ is an equivalence relation on $A$. Then the function $\phi:A\to A/\mathord\sim$ defined via $\phi(x)=[x]$ is surjective.\footnote{Recall that $A/\mathord\sim$ is the set of equivalence classes induced by the equivalence relation $\sim$.}
\end{theorem}
