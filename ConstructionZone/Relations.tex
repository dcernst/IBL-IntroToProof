...
\begin{definition}
Let $A$ and $B$ be sets. A \textbf{relation $\sim$ from a set $A$ to a set $B$} is a subset of $A \times B$. If $\sim$ is a relation from $A$ to $B$ and $(a,b)\in {\sim}$, then we say that \textbf{$a$ is related to $b$}.  In this case, we may also write $a\sim b$. If $\sim$ is a relation from $A$ to the same set $A$, then we say that $\sim$ is a \textbf{relation on $A$}.
\end{definition}
...
It is important to notice that the order in which we write things for relations matters.  In particular, if $\sim$ is a relation from $A$ to $B$ and $a \sim b$, then it may or may not be the case that $b\sim a$.
...
\]
is an example of a relation from $A$ to $B$. In this case, we could write $(c,2)\in{\sim}$ or $c\sim 2$. We could also say that $a$ is related to 1, 2, and 4. 
\end{example}
...
Let $A=\{a,b,c,d\}$ and define $\sim$ on $A$ via
\[
{\sim}=\{(a,a),(a,b),(a,c),(b,b),(b,a),(b,c),(c,c),(c,a),(c,b),(d,d)\}.
\]  
...
Often it is not feasible to draw the complete diagram for the digraph of a relation (e.g., when the sets involved are infinite).  However, sometimes we can still provide a visual representation of the relation by plotting points that are in the relation. The \textbf{graph} of a relation $\sim$ from $A$ to $B$ is the visual representation that results from plotting all points $(a,b)$ that satisfy $a\sim b$.
...
\[
\rel(a,\sim):=\{b\in A\mid a\sim b\}.
\]
We also define the \textbf{collection of the sets of relatives with respect to $\sim$} by
\[
\Rel(\sim):=\{\rel(a)\mid a\in A\}.
\]
\end{definition}

If $\sim$ is clear from the context, we will usually write $\rel(a)$ in place of $\rel(a,\sim)$.  In terms of digraphs, $\rel(a)$ is the collection of vertices that have a directed edge pointing towards them from the vertex labeled by $a$. In graph theory, this collection of vertices is called the \textbf{out neighborhood} of $a$ and each such vertex is called an \textbf{out neighbor}. Notice that $\Rel(\sim)$ is a set of sets.  In particular, an element in $\Rel(\sim)$ is a subset of $A$---equivalently, an element of $\mathcal{P}(A)$.

\begin{example}
Consider the relation given in Example~\ref{ex:relation on finite}.  By inspecting the ordered pairs in $\sim$ or by looking at the digraph in Figure~\ref{fig:digraph on finite}\subref{fig:digraphB}, we see that
\[
\rel(a) = \{a,b,c\},\ \rel(b) = \{a,b,c\},\ \rel(c) = \{d,e\},\ \rel(d) = \{a,c,d\},\ \rel(e) = \{a\},
\]
so that $\Rel(\sim) = \{\{a,b,c\},\{d,e\},\{a,c,d\},\{a\}\}$.
\end{example}

\begin{problem}
Consider the relation given in Problem~\ref{prob:another relation on finite}(a).  Find $\Rel(\sim)$ by determining $\rel(x)$ for each $x\in A$.
\end{problem}
...
Let $P$ denote the set of all people with accounts on Facebook and define the relation $F$ on $P$ via $xFy$ if and only if $x$ is friends with $y$. Describe $\rel(\text{Bob})$, where Bob is the name of a specific Facebook user.  What is $\Rel(F)$?
\end{problem}
...
Suppose $\sim$ is a relation on $A=\{1,2,3,4,5\}$ such that $\rel(1)=\{1,3,4\}$, $\rel(2)=\{4\}$, $\rel(3)=\{3,4,5\}$, $\rel(4)=\{1,2\}$, and $\rel(5)=\emptyset$. List the ordered pairs in $\sim$ and draw the corresponding digraph.
\end{problem}
...
\begin{enumerate}[label=\textrm{(\alph*)}]
\item The relation $\sim$ is \textbf{reflexive} if for all $a\in A$, $a\sim a$.
...
\begin{problem}
Determine whether the two relations given in Problem~\ref{prob:another relation on finite} are reflexive, symmetric, or transitive.
\end{problem}

\begin{problem}
Suppose $\sim$ is a relation on a set $A$.
\begin{enumerate}[label=\textrm{(\alph*)}]
\item Explain what it means for $\sim$ to \emph{not} be reflexive.
\item Explain what it means for $\sim$ to \emph{not} be symmetric.
\item Explain what it means for $\sim$ to \emph{not} be transitive.
\end{enumerate}
\end{problem}
