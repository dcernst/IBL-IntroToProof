...
\begin{problem}\label{prob:relation on finite}
...
\begin{problem}
Consider the relation given in Problem~\ref{prob:relation on finite}(a).  Find $\Rel(\sim)$ by determining $\rel(x)$ for each $x\in A$.
\end{problem}

\begin{problem}
Describe the collection of the set of relatives with respect to the empty relation from Problem~\ref{prob:relation on finite}(b).
\end{problem}
...
Define the relation $\equiv_5$ on $\mathbb{Z}$ via $a\equiv_5 b$ if and only if $a-b$ is a multiple of 5. Find $\rel(1)$, $\rel(2)$, and $\rel(6)$.  How many distinct sets are in $\Rel(\equiv_5)$?
\end{problem}
...
\begin{problem}
Determine whether the two relations given in Problem~\ref{ex:relation on finite} are reflexive, symmetric, or transitive.
\end{problem}
