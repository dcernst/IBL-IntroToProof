...
\begin{definition}
Let $A$ and $B$ be sets. A \textbf{relation $\sim$ from $A$ to $B$} is a subset of $A \times B$. If $\sim$ is a relation from $A$ to $B$ and $(a,b)\in {\sim}$, then we say that \textbf{$a$ is related to $b$}.  In this case, we may write $a\sim b$ in place of $(a,b)\in{\sim}$. If $\sim$ is a relation from $A$ to the same set $A$, then we say that $\sim$ is a \textbf{relation on $A$}.
\end{definition}
