\begin{section}{Sets}\label{sec:Sets}
...
There are a few sets that are commonly discussed in mathematics and have predefined symbols to denote them. We have already encountered the integers, natural numbers, and real numbers.
\begin{itemize}
...
\item \textbf{Rational Numbers:} $\tcboxmath{\mathbb{Q}\coloneqq \{x\in \mathbb{R} \mid \text{there exists }a, b \in \mathbb{Z} \text{ such that } b \neq 0\text{ and }x=a/b\}}$.
...
\end{itemize}
Since the set of natural numbers consists of the positive integers, the natural numbers are sometimes denoted by $\tcboxmath{\mathbb{Z}^+}$. Notice that our definition of the rational numbers uses set-builder notation. This is often abbreviated as
\[
\mathbb{Q} = \{a/b \mid a, b \in \mathbb{Z} \text{ and } b \neq 0\}.
\]
Take a moment to convince yourself that this is a reasonable shorthand.
...
\begin{theorem}\label{thm:trivial subsets}
Let $A$ be a set.  Then
\begin{enumerate}[label=\textrm{(\alph*)}]
...
Observe that ``$A\subseteq B$" is equivalent to ``For all $x$ (in the universe of discourse), if $x\in A$, then $x\in B$."  Since we know how to deal with ``for all" statements and conditional propositions, we know how to go about proving $A\subseteq B$. If $A$ happens to be the empty set, then the statement ``For all $x$ (in the universe of discourse), if $x\in A$, then $x\in B$" is vacuously true.  This is in agreement with Theorem~\ref{thm:trivial subsets}(b), which states that the empty set is always a subset of every set.  In light of this, it is common to omit discussion of the case when $A$ is the empty set when proving that $A$ is a subset of $B$.
