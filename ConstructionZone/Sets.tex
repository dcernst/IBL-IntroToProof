...
\begin{definition}
A \textbf{set} is a collection of objects called \textbf{elements}. If $A$ is a set and $x$ is an element of $A$, we write $x\in A$. Otherwise, we write $x\notin A$. The set containing no elements is called the \textbf{empty set}, and is denoted by the symbol $\emptyset$. Any set that contains at least one element is referred to as a \textbf{nonempty set}.
\end{definition}
...
\begin{itemize}
\item \textbf{Natural numbers:} $\mathbb{N}:=\{1,2,3,\ldots\}$. Some books will include zero in the set of natural numbers, but we will not do that.
