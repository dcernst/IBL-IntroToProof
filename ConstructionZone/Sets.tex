...
\begin{definition}
A \textbf{set} is a collection of objects called \textbf{elements}. If $A$ is a set and $x$ is an element of $A$, we write $x\in A$. Otherwise, we write $x\notin A$. The set containing no elements is called the \textbf{empty set}, and is denoted by the symbol $\emptyset$. Any set that contains at least one element is referred to as a \textbf{non-empty set}.
\end{definition}
...
Every non-empty set always has two rather boring subsets.
