...
Note that if we want to prove $A=B$, then we have to do two separate sub-proofs: one for $A\subseteq B$ and one for $B\subseteq A$. Be sure to make it clear to the reader where these sub-proofs begin and end. One approach is to label each sub-proof with ``$(\subseteq)$" and ``$(\supseteq)$" (including the parentheses), respectively.
