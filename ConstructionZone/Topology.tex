...
The upshot of Parts~(i) and (l) of Problems~\ref{prob:open or not} and \ref{prob:closed or not} is that $\mathbb{R}$ and $\emptyset$ are both open and closed.  It turns out that these are the only two subsets of the real numbers with this property. One issue with terminology that could potentially create confusion is that the open interval $(-\infty, \infty)$ is both an open and a closed set.
...
%???Move this (up to but not including compact) to previous section and prove Archimedean Property from CA???
%
%\begin{definition}
%Let $A\subseteq \mathbb{R}$. A point $b$ is called an \textbf{upper bound} of $A$ if for all $x\in A$, $x\leq b$. The set $A$ is said to be \textbf{bounded above} if it has an upper bound.
%\end{definition}
%
%\begin{problem}
...
%\end{definition}
%
%Notice that a set $A\subseteq\mathbb{R}$ is bounded if it is a subset of some bounded closed interval.
%
%\begin{definition}
%A point $p$ is a \textbf{supremum} (or \textbf{least upper bound}) of $A\subseteq \mathbb{R}$ if $p$ is an upper bound of $A$ and $p\leq b$ for every upper bound $b$ of $A$.  Analogously, a point $p$ is an \textbf{infimum} (or \textbf{greatest lower bound}) of $A\subseteq \mathbb{R}$ if $p$ is a lower bound of $A$ and $p\geq b$ for every lower bound $b$ of $A$. 
%\end{definition}
%
%Our next result tells us that if supremum and infimums are unique when they exist.
%
%\begin{theorem}
...
%\end{theorem}
%
%In light of the previous theorem, if the supremum of $A$ exists, it is denoted $\boxed{\sup(A)}$. Similarly, if the infimum of $A$ exists, it is denoted $\boxed{\inf(A)}$. 
%
%\begin{problem}
%Find the supremum and the infimum of each of the sets in Problem~\ref{prob:find upper bounds}.
%\end{problem}
%
%If $\sup(A)$ exists, it is the minimum of the set of upper bounds of $A$. That is, the set of upper bounds of $A$ is $[\sup(A),\infty)$. We have a similar statement involving the infimum. It is important to recognize that the supremum (respectively, infimum) of a set may or may not be contained in the set. In particular, we have the following theorem concerning supremums and infimums. The analogous result holds for minimum and infimum.
%
%\begin{theorem}
%Let $A\subseteq \mathbb{R}$. Then $A$ has a maximum if and only if $A$ has a supremum and $\sup(A)\in A$, in which case the maximum of $A$ is $\sup(A)$.
%\end{theorem}
%
...
%
%\begin{problem}
%Prove that the Archimedean Property (Axiom~(O7)) follows from the Completeness Axiom.
%\end{problem}
