...
When $\sim$ is an equivalence relation on a set $A$, it is common to write each equivalence class $\rel(a)$ as $[a]$ (or $\overline{a}$). In addition, the collection of equivalence classes $\Rel(\sim)$ is often denoted by $A/\mathord\sim$, which is read as ``$A$ modulo $\sim$" or ``$A$ mod $\sim$".  The collection $A/\mathord\sim$ is sometimes referred to as the \textbf{quotient set of $A$ by $\sim$}.
