...
\begin{problem}
Let $\mathcal{F}$ denote the set of functions from $\mathbb{N}$ to $\{0,1\}$. Prove that $\mathcal{F}$ and $\mathcal{P}(\mathbb{N})$ have the same cardinality.\footnote{\emph{Hint:} Define $\phi:\mathcal{F}\to \mathcal{P}(\mathbb{N})$ so that $\phi(f)$ outputs a subset of $\mathbb{N}$ determined by when $f$ outputs a 1.}
\end{problem}
...
%We write $\boxed{\card(A)< \card(B)}$ provided $\card(A)\leq \card(B)$ and $\card(A)\neq \card(B)$.
\end{definition}
