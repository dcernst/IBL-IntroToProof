...
\begin{example}
Let $P_f$ denote the set of all people with accounts on Facebook.  Define  $F$ via $xFy$ if and only if $x$ is friends with $y$.  Then $F$ is a relation on $P_f$.
\end{example}

We can often represent relations using graphs or digraphs.  Given a finite set $X$ and a relation $\sim$ on $X$, a \textbf{digraph} (short for \emph{directed graph}) is a discrete graph having the members of $X$ as vertices and a directed edge from $x$ to $y$ if and only if $x\sim y$.
...
\begin{enumerate}[label=\textrm{(\alph*)}]
\item Let $P_f$ denote the set of all people with accounts on Facebook.  Define  $F$ via $xFy$ if and only if $x$ is friends with $y$. 
