...
Quantification and logical connectives (``and,'' ``or,'' ``If\ldots, then\ldots,'' and ``not'') enable complex mathematical statements. For example, the formal definition of $\lim_{x\to c}f(x)=L$, which you may have encountered in calculus, is 
\begin{quote}
...
The symbolic expression $(\forall x)(\forall y)$ can be replaced with the simpler expression $\boxed{\forall x,y}$ as long as $x$ and $y$ are elements of the same universe.
