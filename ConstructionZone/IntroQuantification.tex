\begin{section}{Introduction to Quantification}\label{sec:Introduction_to_Quantification}
...
\begin{itemize}
\item $S(x)\coloneqq \text{``}x^2-4=0\text{"}$
\item $L(a,b)\coloneqq \text{``}a<b\text{"}$
\item $F(x,y)\coloneqq \text{``}x \mbox{ is friends with } y\text{"}$
\end{itemize}
...
One way we can make propositions out of predicates is by assigning specific values to the free variables.  That is, if $P(x)$ is a predicate and $x_0$ is a specific value for $x$, then $P(x_0)$ is now a proposition that is either true or false.
...
\epigraph{Like what you do, and then you will do your best.}{Katherine Johnson, mathematician}
