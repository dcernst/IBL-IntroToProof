...
The final theorem of this section can be thought of as a generalized version of the Well-Ordering Principle.

\begin{theorem}\label{thm:reverse WOP}
If $A$ is a nonempty subset of the integers and there exists $b\in A^c$ such that $b\geq a$ for all $a\in A$, then $A$ contains a greatest element.
\end{theorem}

In the previous theorem, $b$ is referred to as an upper bound for $A$. We will study upper bounds in Section~\ref{sec:AxiomsRealNumbers}.
