...
%Here is some more evidence that we shouldn't assume the existence of a set of all sets.
%
%\begin{problem}
%If $\mathcal{U}$ is the set of all sets, then what is the relationship between $\mathcal{U}$ and $\mathcal{P}(\mathcal{U})$?  What about $\mathcal{P}(\mathcal{P}(\mathcal{U}))$?\footnote{This is only problematic if we assume that for all sets $A$, $A\notin A$.}
%\end{problem}
%
