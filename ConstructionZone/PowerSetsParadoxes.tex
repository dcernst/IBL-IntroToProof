...
It follows immediately from the definition that $A\subseteq S$ if and only if $A\in\mathcal{P}(S)$.\footnote{Recall that ``iff" is an abbreviation for `if and only if", which is a statement of the form $A\iff B$ for propositions $A$ and $B$. Recall that this is short for both $(A\implies B) \wedge (B\implies A)$.}  For example, if $S=\{a,b\}$, then $\mathcal{P}=\{\emptyset, \{a\}, \{b\}, S\}$.
